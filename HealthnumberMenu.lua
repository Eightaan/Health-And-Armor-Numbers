--[[
    ...
    healthnumber Menu specific code made by Dom
    List:
    Items in line
...
healthnumberMenu = healthnumberMenu or class()
function healthnumberMenu:init()
   local aspect_ratio = RenderSettings.resolution.x / RenderSettings.resolution.y
	...
	if healthnumberMenu.Warning == 1 then
		self:CreateChangeWarning()
	end

    self:GetMenuFromJson(healthnumber._menu_path .. "MainMenu.json", healthnumber._data)

    self:OpenMenu("healthnumber_menu")
end

function healthnumberMenu:CallCallback(item, params)
...
function healthnumberMenu:update(t, dt)
...
function healthnumberMenu:SetAxisTimer(axis, delay, input_delay, input)
...
function healthnumberMenu:Open()
    self._enabled = true
    ...
    if not self._controller then
        self._controller = managers.controller:create_controller("healthnumberMenu", nil, false)
...
function healthnumberMenu:Close()
    self._enabled = false
...
-- Mouse Functions
function healthnumberMenu:mouse_move(o, x, y)
...
function healthnumberMenu:mouse_press(o, button, x, y)
    x, y = self:_convert_mouse_pos(x, y)
    if button == Idstring("0") then
        ...
        elseif self._input then
            if self._input.panel:inside(x, y) then
                --healthnumber:Log(Activate input")
            else
                self:CloseInput()
            end
...
function healthnumberMenu:mouse_release(o, button, x, y)
    if button == Idstring("0") then
        if self._slider then
...
-- Item interaction
function healthnumberMenu:Confirm()
...
function healthnumberMenu:MenuDown()
    if not self._enabled then
        return
    end
    if self._open_choice_dialog then
...
function healthnumberMenu:MenuUp()
    if not self._enabled then
        return
    end
    if self._open_choice_dialog then
...
function healthnumberMenu:MenuLeftRight(change)
    if not self._enabled then
        return
    end
...
function healthnumberMenu:ActivateItem(item, x)
...
function healthnumberMenu:CreateChangeWarning()
	if managers.hud and not self._panel:child("changed_warning") then
		healthnumberMenu.Warning = 1
		local changed_warning = self._panel:text({
			...
			color = Color.red,
			align = "right",
			text = managers.localization:text("healthnumber_warning_desc")
		})
...
function healthnumberMenu:SetMenuItemsEnabled(menu, parent_item_id, enabled)
...
function healthnumberMenu:GetMenu(menu)
    return self._menus[menu]
end

function healthnumberMenu:AnimateItemEnabled(item, enabled)
...
function healthnumberMenu:HighlightItem(item)
...
function healthnumberMenu:UnhighlightItem(item)
    if item.focus_changed_callback then
...
function healthnumberMenu:SetLegends(accept, reset, step)
...
function healthnumberMenu:Cancel()
    if self._open_choice_dialog then
        self:CloseMultipleChoicePanel()
    elseif self._open_color_dialog then
        self:CloseColorMenu(false)
...
function healthnumberMenu:SetItem(item, value, menu)
...
function healthnumberMenu:GetMenuFromJson(path, ...)
    ...
    if file then
        ...
        if content.title == "healthnumber_title" then
            menu_title = menu_title .. " " .. healthnumber.ModVersion
        end
...
function healthnumberMenu:CreateItem(item, items, menu_id, ...)
    ...
    elseif parents then
        if type(parents) == "string" then
            for _, pitem in pairs(items) do
                if pitem.id == parents then
                    enabled = healthnumber.settings.assault_panel[pitem.special_remarks].enabled
                    break
                end
            end
        end
    ...
    elseif item_type == "color_select" then
        value = healthnumber:GetColor(item.value)
...
function healthnumberMenu:CreateOneLineItems(item, items, menu_id, ...)
    local offset = {
        ["label"] = 5,
        ["button"] = 10,
        ["toggle"] = 34,
...
function healthnumberMenu:CreateMenu(params)
...
function healthnumberMenu:OpenMenu(menu, close)
...
function healthnumberMenu:AddItemToMenu(menu_id, item, total_size)
...
function healthnumberMenu:GetLastPosInMenu(menu_id)
    return self._menus[menu_id].len or 50
end

--Label Items
function healthnumberMenu:CreateLabel(params)
...
--Divider Items
function healthnumberMenu:CreateDivider(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
...
--Button Items
function healthnumberMenu:CreateButton(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
...
--Toggle Items
function healthnumberMenu:CreateToggle(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
    ...
    local check_bg = toggle_panel:bitmap({
        name = "check_bg",
        texture = "guis/textures/pd2_mod_healthnumber/check_bg",
        x = 2,
        y = 2,
        w = 22,
        h = 21,
    ...
    local check = toggle_panel:bitmap({
        name = "check",
        texture = "guis/textures/pd2_mod_healthnumber/check",
        x = 2,
        y = 2,
...
--Slider Items
function healthnumberMenu:CreateSlider(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
...
function healthnumberMenu:SetSlider(item, x, add)
...
function healthnumberMenu:SetInput(item)
    if self._input then
        self:CloseInput()
    end
    self._input = item
    --healthnumber:Log("Input")
...
function healthnumberMenu:key_press(item, o, k)
...
function healthnumberMenu:enter_text(item, o, s)
    local text = item.value
    local m = item.limit
    local n = utf8.len(text)
    s = utf8.sub(s, 1, m - n)
...
function healthnumberMenu:key_release(item, o, k)
    if healthnumber:IsOr(k, Idstring("esc"), Idstring("enter")) then
        self:CloseInput()
    end
end

function healthnumberMenu:CloseInput()
    if self._input then
...
function healthnumberMenu:CreateMultipleChoice(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
...
function healthnumberMenu:OpenMultipleChoicePanel(item)
...
function healthnumberMenu:CloseMultipleChoicePanel()
...
function healthnumberMenu:CreateInput(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
    ...
    input_panel:bitmap({
        texture = "guis/textures/pd2_mod_healthnumber/hud_corner",
        name = "left_top",
        visible = true,
        layer = 0,
        y = 0,
...
-- Custom Color Items
function healthnumberMenu:CreateColorSelect(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
    local texture = params.texture or nil
    local texture_extension = "_healthnumber"
...
function healthnumberMenu:OpenColorMenu(item)
    ...
    red_panel:text({
        ...
        font_size = 18,
        font = tweak_data.menu.pd2_small_font,
        text = managers.localization:text("healthnumber_r"),
        x = 85,
        w = red_panel:w() - 90,
        h = 25,
        align = "right",
    ...
    green_panel:text({
        ...
        font_size = 18,
        font = tweak_data.menu.pd2_small_font,
        text = managers.localization:text("healthnumber_g"),
        x = 85,
        w = red_panel:w() - 90,
        h = 25,
        align = "right",
    ...
    blue_panel:text({
        ...
        font_size = 18,
        font = tweak_data.menu.pd2_small_font,
        text = managers.localization:text("healthnumber_b"),
        x = 85,
        w = red_panel:w() - 90,
        h = 25,
        align = "right",
    ...
    accept_panel:text({
        ...
        font_size = 18,
        font = tweak_data.menu.pd2_small_font,
        text = managers.localization:text("healthnumber_ok"),
        x = 5,
        w = red_panel:w() - 10,
        h = 25,
        align = "right",
    ...
    reset_panel:text({
        ...
        font_size = 18,
        font = tweak_data.menu.pd2_small_font,
        text = managers.localization:text("healthnumber_color_reset"),
        x = 5,
        w = red_panel:w() - 10,
        h = 25,
        align = "right",
...
function healthnumberMenu:SetColorSlider(item, x, type, add)
...
function healthnumberMenu:CloseColorMenu()
...
function healthnumberMenu:ResetColorMenu()
...
function healthnumberMenu:SetOption(value, option)
    healthnumber._data[option] = value
end

function healthnumberMenu:SetColorOption(color, option)
    local c = healthnumber._data[option]
    c.r = color.red
    c.g = color.green
    c.b = color.blue
end